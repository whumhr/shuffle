...
class ScrapyProjectPipeline:
    ...
    def process_item(self, item, spider):
        ...
        if self.baseMapper.contains_value('seed_words','words',item['word']):
            ...
            data = {"_id": id + 1, "words": item['word'], "explanation": None, "url": None, "is_read": False}
        ...
        for j in tags:
            if self.baseMapper.query_count('seed_words') < 200 and not self.baseMapper.contains_value('seed_words','words',j) and self.baseMapper.contains_value('match_words','words',j) and not self.baseMapper.contains_value('stop_words','words',j):  # 分词在高频词但是不在种子词中，则加入
                ##获取字段最大id值
                id = self.baseMapper.query_max_id('seed_words')
