...
class ScrapyProjectPipeline:
    ...
    def process_item(self, item, spider):
        ...
        for j in tags:  # 判断分词是否在高频词里面
            if self.baseMapper.query_count('seed_words') < 200 and self.baseMapper.contains_value('match_words','words',j) and not self.baseMapper.contains_value('stop_words','words',j):  # 分词在高频词但是不在种子词中，则加入
                #获取字段最大值
